...
import {
  Activity, Search, ArrowRight, Droplets, Flame, Anchor, Zap, Wind, BarChart3
} from "lucide-react";
...
export default function RebhXRayLandingPage() {
  ...
  const go = (sym: string) => router.push(`/rebh/xray/${sym.trim().toUpperCase()}`);
      ...
      <main className="max-w-5xl mx-auto px-6 py-10 space-y-10">
        ...
        <section className="bg-white border border-[#E5E7EB] rounded-[8px] p-8 shadow-[0_1px_4px_rgba(0,0,0,0.06)] space-y-4">
          <h2 className="text-xl font-black text-[#1A1A1A] tracking-tight">
            قراءة X-Ray لأي شركة سعودية مُدرجة
          </h2>
          <p className="text-xs text-[#6B7280]">
            يُولِّد المحرك قصة مالية متكاملة من الأرقام الحقيقية — نهر المال · الميزانية · الهوامش · الديون · جودة الأرباح.
            كل جملة مربوطة بصيغة ومصدر.
          </p>
          <form onSubmit={handleSubmit} className="flex items-center gap-2">
            <div className="relative flex-1 max-w-xs">
              <Search size={14} className="absolute right-3 top-1/2 -translate-y-1/2 text-[#9CA3AF] pointer-events-none" />
              <input type="text" inputMode="numeric" maxLength={4} value={input}
        ...
        <section className="space-y-3">
          <h3 className="text-sm font-bold text-[#374151] flex items-center gap-2">
            <Droplets size={15} className="text-[#8C3B32]" />
            ما يحتويه التقرير
          </h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {WHAT_INSIDE.map((w, i) => (
              <div key={i} className="bg-white border border-[#E5E7EB] rounded-[6px] p-4 flex items-start gap-3 hover:border-[#8C3B32] hover:shadow-sm transition-all">
                ...
                <div>
                  <span className="block text-xs font-bold text-[#0F172A]">{w.title}</span>
                  <span className="block text-[11px] text-[#64748B] mt-0.5">{w.desc}</span>
                </div>
              </div>
            ))}
          </div>
